...
class _LoginPageState extends State<LoginPage>{
  ...
  Future<void> submit() async {
    setState((){ //default state, waiting for input, no current errors
      _isLoading = true;
      _errorMessage = null;
    });
    ...
    try{
      ...
      } else { //want to add specific message if email exists but password is wrong vs email dne
        final body = jsonDecode(response.body);
        setState((){
          _errorMessage = body['message'] ?? "Login failed. Please check your credentials.";
        });
      }
    }
    catch (e) {
    ...
    } finally { //mark end of loading regardless of outcome
      setState(() => _isLoading = false);
    }
  }

  Future<void> _sendPasswordReset() async { //process for resetting password
    final email = emailController.text.trim();
    ...
    final url = Uri.parse("http://10.0.2.2:5000/reset-password"); //add logic to allow password reset (user can reset password for given email?)
    ...
    try{
      final response = await http.post( //begin process to send reset link
        url,
        headers: {"Content-Type": "application/json"},
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(body['message'] ?? "Attempting to send reset link")),
      ); 
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_errorMessage != null)...[
              ...
              Text(
              _errorMessage!,
              style: const TextStyle(color: Colors.red, fontSize: 14) //text style for error messages
              ),
            ],
            ...
            Align( //button for password reset
            ...
            TextButton( //button to route to register page instead
              onPressed: (){
              Navigator.pushReplacement( 
              context,
